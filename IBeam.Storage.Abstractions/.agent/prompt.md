# IBeam.Storage.Abstractions Agent Prompt

You are working inside `IBeam.Storage.Abstractions`.
...
Provider-neutral file/blob storage abstractions for IBeam.
...
- Blob/file models
- Storage options and result contracts
...
- Keep this package independent from Azure, S3, and file-system implementations.
- Do not add API controllers or service business rules here.
- Design abstractions that services can call while keeping permissions and audit in the service layer.
- Keep provider-specific credentials out of abstractions.
...
Add or update focused tests in `IBeam.Tests.Storage.Abstractions` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
