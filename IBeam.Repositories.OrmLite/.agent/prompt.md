# IBeam.Repositories.OrmLite Agent Prompt

You are working inside `IBeam.Repositories.OrmLite`.
...
ServiceStack OrmLite repository infrastructure for IBeam entity persistence.
...
- OrmLite repository base classes
- OrmLite connection/registration helpers
- SQL persistence conventions
...
- Keep OrmLite mechanics isolated here.
- Do not add domain-specific authorization or audit rules.
- Map one repository to one entity-focused persistence boundary.
- Preserve behavior parity with core repository abstractions.
...
Add or update focused tests in `IBeam.Tests.Repositories.OrmLite` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
