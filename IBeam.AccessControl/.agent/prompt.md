# IBeam.AccessControl Agent Prompt

You are working inside `IBeam.AccessControl`.
...
Core access-control contracts and models for IBeam operation permissions, role grants, subject grants, agents, and bring-your-own-auth authorization decisions.
...
- Operation/role/subject abstractions
- Access-control result contracts
- Extension points that do not require IBeam.Identity
...
- Keep this package independent from Identity so teams can use their own auth system.
- Model permissions around stable IBeam operation names like pricing.save or purchases.delete.
- Keep dynamic storage and API management concerns in the repository/service/API packages.
- Avoid application-specific roles or tenant rules in this core package.
...
Add or update focused tests in `IBeam.Tests.AccessControl` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
