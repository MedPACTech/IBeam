# IBeam.Communications.Email.SendGrid Agent Prompt

You are working inside `IBeam.Communications.Email.SendGrid`.
...
SendGrid email provider for the shared IEmailService abstraction.
...
- SendGrid options
- DI registration for SendGrid transport
...
- Keep SendGrid SDK/API details isolated here.
- Use shared EmailMessage validation and sender resolution.
- Do not add templates unless they are provider-specific transport features and still flow through IEmailService.
...
Add or update focused tests in `IBeam.Tests.Communications.Email.SendGrid` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
