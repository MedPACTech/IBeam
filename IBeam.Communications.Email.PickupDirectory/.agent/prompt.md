# IBeam.Communications.Email.PickupDirectory Agent Prompt

You are working inside `IBeam.Communications.Email.PickupDirectory`.
...
File-system pickup-directory email provider for development, diagnostics, and environments that hand messages to another mail process.
...
- Pickup directory email service
- Pickup directory options
- DI registration for file-based email output
...
- Write messages as transport artifacts, not business audit records.
- Use shared email validation and sender resolution.
- Do not add SMTP or external provider behavior here.
- Keep file paths configurable and safe for local/integration use.
...
Add or update focused tests in `IBeam.Tests.Communications.Email.PickupDirectory` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
