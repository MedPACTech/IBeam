# IBeam.Identity.Api Agent Prompt

You are working inside `IBeam.Identity.Api`.
...
HTTP API surface for IBeam Identity account, tenant, role, OTP, and credential workflows.
...
- Identity controllers/endpoints
...
- Keep controllers thin and delegate to IBeam.Identity.Services.
- Do not put password, OTP, tenant, role, or credential business rules in controllers.
- Use base API response/error helpers.
- Keep API DTOs separate from storage entities.
...
Add or update focused tests in `IBeam.Tests.Identity.Api` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
