# IBeam.Communications.Email.AzureCommunications Agent Prompt

You are working inside `IBeam.Communications.Email.AzureCommunications`.
...
Azure Communication Services email provider for the shared IEmailService abstraction.
...
- Azure Communications email service
- Azure email options
- DI registration for Azure email transport
...
- Keep this package as a provider adapter only.
- Do not add API controllers, repositories, identity dependencies, or templating engines.
- Use shared EmailMessage validation and sender resolution.
- Wrap provider failures in IBeam communication exceptions.
...
Add or update focused tests in `IBeam.Tests.Communications.Email.AzureCommunications` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
