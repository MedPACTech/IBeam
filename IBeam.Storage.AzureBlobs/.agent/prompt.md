# IBeam.Storage.AzureBlobs Agent Prompt

You are working inside `IBeam.Storage.AzureBlobs`.
...
Azure Blob Storage provider for IBeam storage abstractions.
...
- Azure Blob options
- DI registration for Azure storage
...
- Keep Azure SDK details isolated here.
- Do not make service authorization decisions in the provider.
- Use provider-neutral storage abstractions for callers.
- Keep credentials and container settings configurable.
...
Add or update focused tests in `IBeam.Tests.Storage.AzureBlobs` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
