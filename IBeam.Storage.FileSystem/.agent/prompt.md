# IBeam.Storage.FileSystem Agent Prompt

You are working inside `IBeam.Storage.FileSystem`.
...
Local or mounted file-system storage provider for IBeam storage abstractions.
...
- DI registration for local storage
...
- Keep path handling safe and configurable.
- Do not add application-specific file organization rules here.
- Return provider-neutral storage results to callers.
- Keep permissions and audit in the service layer that requests storage operations.
...
Add or update focused tests in `IBeam.Tests.Storage.FileSystem` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
