# IBeam.Ai.Services Agent Prompt

You are working inside `IBeam.Ai.Services`.
...
Service-layer orchestration for IBeam AI workflows, provider calls, validation, logging, and permission-aware operation execution.
...
- Provider-facing service abstractions
- Operation tags for AI calls
...
- Keep AI business rules and provider orchestration here.
- Use stable operation names for AI calls so access control and audit can target them.
- Do not let API projects bypass this service layer.
- Keep repository usage entity-focused if AI state or history is persisted.
...
Add or update focused tests in `IBeam.Tests.Ai.Services` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
