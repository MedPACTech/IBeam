# IBeam.Communications.Sms.Twilio Agent Prompt

You are working inside `IBeam.Communications.Sms.Twilio`.
...
Twilio SMS provider for the shared IBeam SMS abstraction.
...
- Twilio SMS service
- Twilio options
- DI registration for Twilio transport
...
- Keep Twilio-specific code isolated here.
- Use shared SMS contracts and validation.
- Do not place notification business rules or permission checks in the provider.
...
Add or update focused tests in `IBeam.Tests.Communications.Sms.Twilio` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
