# IBeam.AccessControl.Api Agent Prompt

You are working inside `IBeam.AccessControl.Api`.
...
API endpoints and request/response DTOs for managing IBeam access-control roles, grants, and permission maps when a team chooses dynamic management.
...
- Access-control controllers/endpoints
- Role and permission management DTOs
...
- Keep controllers thin and delegate all permission logic to IBeam.AccessControl.Services.
- Do not evaluate authorization rules in the API layer.
- Use base API response/error helpers.
- Treat this package as optional; scripting/config-only teams should not need it.
...
Add or update focused tests in `IBeam.Tests.AccessControl.Api` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
