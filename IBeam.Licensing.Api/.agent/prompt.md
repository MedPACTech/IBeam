# IBeam.Licensing.Api Agent Prompt

You are working inside `IBeam.Licensing.Api`.
...
Thin API layer for IBeam licensing workflows and entitlement checks exposed over HTTP.
...
- Licensing controllers/endpoints
...
- Delegate license validation and entitlement rules to IBeam.Licensing.Services.
- Do not evaluate license policy in controllers.
- Use base API response/error helpers.
- Keep endpoints optional for teams that validate licenses internally.
...
Add or update focused tests in `IBeam.Tests.Licensing.Api` when behavior changes. If that test project does not exist yet, use the closest existing test project for this package family.
